...
var imgList = ['assets/t_b_white-space.png','assets/space.png','assets/space.png','assets/space.png','assets/test.png','assets/sky.png']
for(var i=0;i<6;i++){
...
function CheckImageLoaded(){
    ...
    if(imgloaded == imgCount){
      ...
      setTimeout(()=>{document.getElementsByClassName('loading_screen')[0].style.top = '150%'},500)
    }
}
...
document.querySelectorAll('.scroll_divs').forEach((ele)=>{
  scroll_vals.push({'top':ele.offsetTop ,'height':ele.clientHeight});
});
...
function toggleGifScroll(id,scroll_pos){
  if(scroll > getPositionHeight(scroll_vals[Math.floor(scroll_pos/100)].top,scroll_vals[Math.floor(scroll_pos/100)].height,scroll_pos%100)){
    setTimeout(()=> {document.getElementById(id).setAttribute('y',530);},500);
    document.getElementById(id).animate({'y':530},{direction:'normal',duration:510,easing:'ease'});    
  }
  else{
    setTimeout(()=> {document.getElementById(id).setAttribute('y',446);},500);
    document.getElementById(id).animate({'y':446},{direction:'normal',duration:510,easing:'ease'});  
  }
...
//fliping card when sent button is clicked
document.querySelector('.contact_btn').addEventListener('click' , (e)=>{
  e.preventDefault();
  document.querySelector('.ground_contact').style.transform = 'translateY(-50%) translateX(-50%) rotateY(180deg)';
  setTimeout(()=>{
    document.querySelector('.ground_contact').style.transform = 'translateY(-50%) translateX(-50%) rotateY(0deg)';
  },2000)
});
